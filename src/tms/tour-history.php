...
if(isset($_REQUEST['bkid']))
{
    ...
    $sql_check = "SELECT status FROM tblbooking WHERE UserEmail=:email AND BookingId=:bid";
    ...
    if ($result->status == 1) {
        ...
        $error = "Đơn hàng đã được xác nhận, bạn không thể hủy.";
    } else {
        // Cho phép hủy nếu trạng thái không phải là Đã xác nhận
        $status = 2;
        $cancelby = 'u';
        ...
        $msg = "Đã hủy đặt dịch vụ thành công";
    }
}
...
<div class="banner-1 ">
	<div class="container">
		<h1 class="wow zoomIn animated animated" data-wow-delay=".5s" style="visibility: visible; animation-delay: 0.5s; animation-name: zoomIn;">Dịch vụ sửa chữa nhà cửa</h1>
	</div>
</div>
<!--- /banner-1 ---->
<!--- privacy ---->
<div class="privacy">
	<div class="container">
		<h3 class="wow fadeInDown animated animated" data-wow-delay=".5s" style="visibility: visible; animation-delay: 0.5s; animation-name: fadeInDown;">Lịch sử của tôi</h3>
...
{
echo "Hệ thống đã hủy vào lúc " .$result->upddate;
} 
